...
test('html with provided template and filterBody callback',function () {
  var _notification = merge(notification,{
    html: {
      template: '<p>bar: <%= body.bar %></p>',
  ...
  return makeOptions({notification:_notification,body})
    .then(function (options) {
      expect(options.html).toMatch(`<p>bar: asdf`);
    });
});

test('html with provided template and filterBody callback that returns promise',function () {
  var _notification = merge(notification,{
    html: {
      template: '<p>bar: <%= body.bar %></p>',
      bodyFilter: function (body) {
